...
// ZSDCC
// zcc +yaz180 -subtype=app -v -m -SO3 --list --math32_z180  -llib/yaz180/th02 -llib/yaz180/i2c_lcd --c-code-in-asm --max-allocs-per-node200000 i2c_life.c -o i2c_life -create-app

// SCCZ80
// zcc +yaz180 -subtype=app -clib=new -v -m -SO3 --list --math32_z180 -llib/yaz180/th02 -llib/yaz180/i2c_lcd i2c_life.c -o i2c_life -create-app
...
void fill_random_data(uint8_t *to)
{
    ...
    srand( (uint16_t)_system_time_fraction*256 + _system_time_fraction );
...
void main(void)
{
    ...
    LCD_Init(I2C2_PORT);
    printf("LCD Port: 0x%.2X\n",LCD_Port);

    th02_init(I2C1_PORT);

    temperature = th02_read_temperature(I2C1_PORT);
    printf("Temperature: %gC\n",temperature);

    humidity = th02_read_humidity(I2C1_PORT);
    printf("Humidity: %g%%\n",humidity);

    LCD_WorkingModeConf(ON, ON, WM_RamMode);// Start RAM mode (characters won't work).
