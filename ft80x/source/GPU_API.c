...
//  vTaskDelay( 32 / portTICK_PERIOD_MS ); // assuming waking from POWERDOWN or SLEEP. From STANDBY the delay is unnecessary.
    ...
    GPU_HostCommand(pscreen, GPU_CORE_RESET);
    ...
    GPU_HAL_Wr8(pscreen,  REG_INT_EN, 0x01);   // Global enable for interrupts attached to INT0 on 328p or 1284p
...
ft_void_t GPU_API_Touch_Config(ft_void_t)
{
    ...
    GPU_CoCmd_Text(pscreen, DispWidth/2,DispHeight/2,27,OPT_CENTERX|OPT_CENTERY, PSTR("Please tap on each dot!"));
